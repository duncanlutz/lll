...
const setMenu = (newNumber, space) => {
    ...
    <div class="profile-section swiper-wrapper">
        ...
        <div class="team-profile-item pi-inactive swiper-slide">
            <a href="attorney.html?duncan-lutz" class="image-wrapper">
...
const checkWidth = () => {
    ...
    switch (true) {
        case (width < 700): menuNumber = 2; spaceBetween = 99; break;
        case (width > 700 && width < 1200): menuNumber = 3; spaceBetween = 66; break;
        case (width > 1200): menuNumber = 4; spaceBetween = 33; break;
    }
...
$(window).on('resize', () => { requestAnimationFrame(() => startAnimating(5)) });
...
$('body').on('click', (e) => {
    if (e.target.parentElement.classList.contains('accessibility-menu-button') || e.target.classList.contains('accessibility-menu-button') || e.target.classList.contains('accessibility-menu') || e.target.parentElement.classList.contains('accessibility-menu')) {
        return;
    }
...
const hideAccess = (e) => {
    if (e[0].isIntersecting) {
        ...
        setTimeout(() => { if (!$('.accessibility').hasClass('access-hidden')) { return }; $('.accessibility').css('display', 'none'); }, 500);
        return;
    }
...
const openSelectMenu = (b) => {
    ...
    if (!$('.options-wrapper').hasClass('select-menu-active') && $('.options-wrapper').css('display') === 'unset') {
        return;
    }
...
const checkForm = (e) => {
    ...
    try {
        ...
        if (name.match(/(?!\s|-|\p{L})\W|\d/gm) != undefined && nameError === false) {
        ...
        if (phone.match(/[a-zA-Z]/gm) != undefined) {
            ...
            addToMessage(`* Please remove ${matchedLines} from the phone number box.`);
        }
        ...
        if (message.val().match(/[a-zA-Z]/gm) == undefined || message.val().length == 0) {
    ...
    const messageFail = () => {
        $('.error-box').append('<div class="message-error">An error occured while submitting your message. Please try again or call us at <a href="tel:8014466464">(801) 446-6464</a>');
    }

    const form = document.querySelector('form');

    emailjs.sendForm('service_qybifle', 'template_gx23nbj', form)
        .then(() => {
            messageSuccess();
        }, error => {
            messageFail();
